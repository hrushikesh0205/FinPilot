...
export function ScannerPage() {
  ...
  const runOCR = async (file) => {
    ...
    try {
      ...
      if (data.potentialDuplicate) {
        setDuplicateWarning(data.duplicateReason || 'A transaction matching this merchant, date, and amount was detected in your history.');
      } else {
  ...
  const handleSaveExpense = async () => {
    ...
    try {
      ...
      toast({
        title: err?.response?.status === 409 ? 'Duplicate Transaction' : 'Save failed',
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {duplicateWarning && (
                <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-800 dark:text-amber-300 text-sm space-y-1">
                  <div className="flex items-center gap-2 font-semibold">
                    <AlertTriangle className="w-4 h-4 flex-shrink-0 text-amber-600" />
                    Possible Duplicate Transaction Detected
                  </div>
                  <p className="text-xs text-muted-foreground dark:text-amber-300/80">{duplicateWarning}</p>
                </div>
              )}
