          ...
          {summary.length > 0 ? (
            <Card>
              ...
              <CardContent className="space-y-5">
                {summary.map((b, i) => {
                  const pct = b.percentageUsed !== undefined && b.percentageUsed !== null
                    ? b.percentageUsed
                    : (b.monthlyLimit > 0 ? Math.round((b.totalSpent / b.monthlyLimit) * 100) : 0);
                  
                  // Status badge helper
                  const getStatusBadge = () => {
                    if (b.statusCode === 'OVER_BUDGET' || b.isOverBudget || pct >= 100) {
                      return <Badge variant="destructive" className="text-xs bg-rose-600">Over budget</Badge>;
                    }
                    if (b.statusCode === 'WARNING' || pct >= 90) {
                      return <Badge className="text-xs bg-amber-500 hover:bg-amber-600 text-white">Warning ({pct}%)</Badge>;
                    }
                    if (b.statusCode === 'APPROACHING' || pct >= 80) {
                      return <Badge className="text-xs bg-yellow-500 hover:bg-yellow-600 text-white">Approaching limit</Badge>;
                    }
                    return <Badge className="text-xs bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">Normal</Badge>;
                  };
                  ...
                  return (
                    <div key={i} className="p-4 rounded-xl border border-border/50 bg-card/40 space-y-2.5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-base">{b.category}</span>
                          {getStatusBadge()}
                        </div>
                        <span className="text-sm font-semibold">
                          ₹{(b.totalSpent || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                          <span className="text-muted-foreground font-normal text-xs ml-1">
                            / ₹{(b.monthlyLimit || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })} ({pct}%)
                          </span>
                        </span>
                      </div>
                      <Progress
                        value={Math.min(pct, 100)}
                        className={cn('h-2.5 bg-muted', getProgressColor())}
                      />
                      <div className="flex justify-between text-xs text-muted-foreground pt-0.5">
                        ...
                        <span className={cn(pct >= 100 ? 'text-rose-500 font-semibold' : 'text-emerald-600 font-medium')}>
                          {pct >= 100 ? 'Limit exceeded' : `₹${(b.remaining || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })} remaining`}
                        </span>
                      </div>
                    </div>
                  );
